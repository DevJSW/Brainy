...
public class ChannelChatroomActivity extends AppCompatActivity {

    String group_id = null;
    String created_date = null;
    String founder_name = null;
    ...
    private TextView mNoNotification, groupName, subscriberCounter;
    ...
    RelativeLayout join_bar, bottomBar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_channel_chatroom);
        Toolbar my_toolbar = (Toolbar) findViewById(R.id.mCustomToolbar);
        setSupportActionBar(my_toolbar);

        Window window = ChannelChatroomActivity.this.getWindow();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.setStatusBarColor(ContextCompat.getColor( ChannelChatroomActivity.this,R.color.colorPrimaryDark));
        ...
        founder_name = getIntent().getStringExtra("founder_name");
        ...
        join_bar = (RelativeLayout) findViewById(R.id.join_bar);
        join_bar.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //ADD USER TO CHANNEL
                DatabaseReference newPost = mDatabaseGroups.child(group_id).child("Participants").child(auth.getCurrentUser().getUid());
                DatabaseReference newPost2 = mDatabaseUsersGroups.child(auth.getCurrentUser().getUid()).child(group_id);

                Map<String, Object> map = new HashMap<>();
                map.put("username", currentuser_username);
                map.put("name", currentuser_name);
                map.put("user_image", currentuser_image);
                map.put("uid", auth.getCurrentUser().getUid());
                newPost.setValue(map);

                //ADD CHANNEL TO USER GROUPS
                Map<String, Object> map2 = new HashMap<>();
                map2.put("group_name", group_name);
                map2.put("group_id", group_id);
                map2.put("group_type", "CHANNEL");
                map2.put("group_image", group_image);
                map2.put("founder_id", founder_id);
               ...
               /* Map<String, Object> map3 = new HashMap<>();
                map3.put("message", currentuser_name+ " joined the channel"+" on "+stringDate2);
        ...
        subscriberCounter = (TextView) findViewById(R.id.toolbar_subscribers);
            ...
            mDatabaseGroupChats.child(group_id).addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    if (dataSnapshot.getValue() == null) {
                        ...
                        mNoNotification.setText("No Messages!");
                    } else {
        ...
        backBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ChannelChatroomActivity.this.finish();
            }
        });
        ...
        groupName = (TextView) findViewById(R.id.toolbar_channel_name);

        if (groupName != null)
            groupName.setText(group_name);

        toolbar_groupimage = (CircleImageView) findViewById(R.id.toolbarImg);
       /* Glide.with(getApplicationContext())
        ...
        //CHECK IF CURRENT USER IS CHANNEL FOUNDER
        if (auth.getCurrentUser().getUid().equals(founder_id)) {
            RelativeLayout typing_bar = (RelativeLayout) findViewById(R.id.typing_bar);
            bottomBar.setVisibility(View.VISIBLE);
            typing_bar.setVisibility(View.VISIBLE);
            join_bar.setVisibility(View.GONE);
        } else {
            mDatabaseGroups.child(group_id).child("Participants").addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    if (dataSnapshot.hasChild(auth.getCurrentUser().getUid())) {
                        ...
                        RelativeLayout typing_bar = (RelativeLayout) findViewById(R.id.typing_bar);
                        bottomBar.setVisibility(View.VISIBLE);
                        typing_bar.setVisibility(View.GONE);
    ...
    private void startPosting() {
        ...
        if (TextUtils.isEmpty(questionBodyTag)) {
            Toast.makeText(ChannelChatroomActivity.this, "Type something!",Toast.LENGTH_LONG).show();
